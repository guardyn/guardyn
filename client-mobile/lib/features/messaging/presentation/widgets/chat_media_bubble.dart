...
/// Widget for displaying media attachments in chat messages
///
/// Handles:
...
class ChatMediaBubble extends StatefulWidget {
  /// The message containing media metadata
  ...
  /// Check if a message has media attachment
  static bool hasMedia(Message message) {
    return message.metadata.containsKey('media_id') &&
        message.metadata['media_id']?.isNotEmpty == true;
  }

  /// Get media ID from message metadata
  static String? getMediaId(Message message) {
    return message.metadata['media_id'];
  }
...
class _ChatMediaBubbleState extends State<ChatMediaBubble> {
  ...
  void _loadMediaMetadata() {
    final mediaId = ChatMediaBubble.getMediaId(widget.message);
