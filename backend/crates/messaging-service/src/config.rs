...
impl MessagingConfig {
    ...
    pub fn from_env() -> Self {
        let bind_address = env::var("BIND_ADDRESS").unwrap_or_else(|_| "0.0.0.0:50052".to_string());
    ...
    pub fn print_summary(&self) {
        ...
        if self.mls.enabled {
            ...
            println!(
                "  - Key Package TTL: {} days",
                self.mls.key_package_ttl_days
            );
        ...
        if self.e2ee.enabled {
            ...
            println!(
                "  - Max Skipped Keys: {}",
                self.e2ee.max_skipped_message_keys
            );
        }
    }
}
...
mod tests {
    ...
    fn test_e2ee_config_defaults() {
        ...
        assert!(!config.enabled);
        assert!(config.x3dh_enabled);
        assert!(config.double_ratchet_enabled);
