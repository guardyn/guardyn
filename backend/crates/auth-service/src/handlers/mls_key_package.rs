...
pub async fn upload_mls_key_package(
    ...
    let user_id = match verify_jwt(&req.access_token, jwt_secret) {
        ...
        Err(e) => {
            ...
            return Ok(Response::new(UploadMlsKeyPackageResponse {
                result: Some(
                    crate::proto::auth::upload_mls_key_package_response::Result::Error(
                        ErrorResponse {
                            code: crate::proto::common::error_response::ErrorCode::Unauthorized
                                as i32,
    ...
    if req.key_package.is_empty() {
        return Ok(Response::new(UploadMlsKeyPackageResponse {
            result: Some(
                crate::proto::auth::upload_mls_key_package_response::Result::Error(ErrorResponse {
    ...
    match db.put(key.as_bytes(), req.key_package).await {
        Ok(_) => {
            info!(
                "MLS key package uploaded: {} for user {}",
                package_id, user_id
            );
            ...
            let _ = db
                .put(list_key.as_bytes(), package_id.as_bytes().to_vec())
                .await;
            ...
            Ok(Response::new(UploadMlsKeyPackageResponse {
                result: Some(
                    crate::proto::auth::upload_mls_key_package_response::Result::Success(
...
pub async fn get_mls_key_package(
    ...
    if req.user_id.is_empty() {
        return Ok(Response::new(GetMlsKeyPackageResponse {
            result: Some(
                crate::proto::auth::get_mls_key_package_response::Result::Error(ErrorResponse {
                    code: crate::proto::common::error_response::ErrorCode::InvalidRequest as i32,
    ...
    match db.get(list_key.as_bytes()).await {
        Ok(Some(package_id_bytes)) => {
            ...
            let key = format!(
                "/mls/key_packages/{}/{}/{}",
                req.user_id, device_id, package_id
            );
            match db.get(key.as_bytes()).await {
                Ok(Some(key_package)) => {
                    info!(
                        "MLS key package found: {} for user {}",
                        package_id, req.user_id
                    );
                    Ok(Response::new(GetMlsKeyPackageResponse {
                        result: Some(
                            crate::proto::auth::get_mls_key_package_response::Result::Success(
                ...
                Ok(None) => {
                    error!(
                        "MLS key package not found: {} for user {}",
                        package_id, req.user_id
                    );
                    Ok(Response::new(GetMlsKeyPackageResponse {
                        result: Some(
                            crate::proto::auth::get_mls_key_package_response::Result::Error(
                                ErrorResponse {
                                    code: crate::proto::common::error_response::ErrorCode::NotFound
                                        as i32,
